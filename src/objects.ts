...
export function isCorrect(question: Question, answer: string): boolean {
    const cleaned_answer: string = answer.trim().toLowerCase();
    const expected_answer: string = question.expected.trim().toLowerCase();

    if (cleaned_answer === expected_answer) {
        return true;
    } else {
        return false;
    }
...
export function isValid(question: Question, answer: string): boolean {
    const question_type: string = question.type;
...
export function toShortForm(question: Question): string {
    const q_ID: string = question.id.toString();
    const first10: string = question.name.slice(0, 10);
    const stringWeWant: string = q_ID + ": " + first10;
    return stringWeWant;
}
...
export function toMarkdown(question: Question): string {
    // need to add the two type of question functionality
